...
public class Arvbin<T extends Comparable<T>>
{
    ...
    public void tornaRaiz(T valor){
        //Não é necessário fazer nada no caso dessa condição ser verdadeira
        if(valor==null || valor.compareTo(val)==0) return;

        //Usa método busca para achar o primeiro nó que tenha o valor passado por parâmetro
        Arvbin<T> temp = this.busca(valor);

        //Se o nó existir, substitui o nó corrente pelo nó com o valor passado como parâmetro
        if(temp !=null) {
            this.val=temp.val;
            this.dir = temp.dir;
            this.esq = temp.esq;
