...
public class Arvbin<T extends Comparable<T>>
{
    ...
    public boolean eSimetrica(){
        ...
        //Testa se acabou os nós da árvore dos dois lados (a partir do nó raiz) ou só em um deles
        if(dir == null && esq == null) return true;
        if(dir == null || esq == null) return false;

        //Testa se os valores dos nós logo abaixo do nó raiz são equivalentes,
        if(dir.val != esq.val) return false;


        //Se os dois lados da árvore chegaram em suas folhas simplesmente retorna true
        if((esq.dir == null && esq.esq == null) && (dir.dir == null && dir.esq == null)) return true;

        //Se um lado da árvore tem mais nós que o outro retorna false. Essa checagem é feita de modo simétrico,ou seja,
        // se os nós direito e esquerdo no lado direito e esquerdo da árvore, respectivamente
        // ("direito.direito" e "esquerdo.esquerdo"), forem ambos nulos ou não nulos, o método continua,
        // pois a árvore continua simétrica. O mesmo vale para os nós esquerdo e direito também em ambos os lados
        // ("direito.esquerdo" e "esquerdo.direito"). Se eles não forem ambos nulos ou não nulos,
        // o método retorna "false"
        if((esq.dir == null && dir.esq != null) || (esq.esq != null && dir.dir == null)) return false;
        if((esq.dir != null && dir.esq == null) || (esq.esq == null && dir.dir != null)) return false;


        //Testa se os valores são iguais nos nós simétricos dos dois lados da árvore, caso os nós não sejam nulos
        if(dir.dir!=null){
            if(esq.esq.val != dir.dir.val) return false;
        }
        if (esq.dir!=null){
            if(esq.dir.val != dir.esq.val) return false;
        }


        boolean simetricaEsquerda = esq.esq.eSimetrica();

        boolean simetricaDireita = dir.dir.eSimetrica();

        return simetricaDireita && simetricaEsquerda;
    }
