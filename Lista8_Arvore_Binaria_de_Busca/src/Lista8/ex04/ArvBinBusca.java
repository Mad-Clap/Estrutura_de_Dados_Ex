...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> 
{
    ...
    */
    ...
    public boolean removeForaIntervalo(Chave chaveMin, Chave chaveMax){
        ...
        //Caso uma das chaves seja nula, ou chaveMax seja menor que chaveMin, ou só haja um nó na árvore,
        // o método retorna false (no último caso porque, mesmo que seja declarada como parâmetro
        // para chaveMin a única chave existente na árvore, nada será removido).
        ...
        //Os booleanos indicam se algo foi removido acima do valor mínimo e/ou abaixo do valor máximo,
        // e eles que são retornados ao final da execução. Caso um deles seja true, o método retorna true.
    ...
    //O método recebe uma das chaves e o booleando chaveMin, que serve justamente para saber se o método
    // está recebendo o objeto Chave chaveMin ou chaveMax. Ele retorna um booleano que indica se o retorno
    // do método removeSubArvore() é nulo ou não. Sendo nulo significa que o outro método rodou a
    // árvore inteira, e não existe nenhum nó com a chave fornecida.
    private boolean removeForaIntervaloAuxiliar(Chave chave, Boolean chaveMin)
    {
        return removeSubArvore(raiz, chave, chaveMin) != null;
    }

    //Esse método é uma modificação do método getchave(), mas ao invés de retornar a chave ou seu valor ele realiza
    // a exclusão das sub-árvores acima e abaixo dos nós com as chaves cujo valor é igual a chaveMin e chaveMax.
    // Ele também recebe um parâmetro a mais que o método getchave(), sendo este o booleano chaveMin.
    private No removeSubArvore(No x, Chave chaveRemove, Boolean chaveMin )
    {
        // Chave não se encontra na árvore.
        if(x == null)
            return null;
        ...
        if(cmp < 0) /* Deve-se ir para a esquerda. */
            ...
            //O booleano chaveMin serve para determinar o que será feito ao encontrar o nó com a chave
            // desejada. Caso o método tenha a chave chaveMin passada como parâmetro,
            // o booleano chaveMin será true, e o nó encontrado passará a ser o nó raiz da árvore.
            // Caso o booleano seja false, o método tem como parâmetro a chave chaveMax,
            // e o método irá excluir as sub-árvores direita e esquerda do nó.
            if(chaveMin) raiz=x;
            else {
                x.dir=null;
                x.esq=null;
