...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> 
{
    ...
    */
    ...
    public void constroiArvore(int[] vetor){
        ...
        int chaveraiz = vetor.length%2==0 ? vetor.length/2 : vetor.length/2+1;
        vetArvore.inserir(chaveraiz, vetor[0]);
        int chave=0;

        for (int j : vetor) {
            if(j==vetor[0]) continue;

            chave++;
           if(chave==chaveraiz) chave++;

            vetArvore.inserir(chave, j);
        }
        this.raiz=(No) vetArvore.raiz;
    }

    // Uma versão do método put() que não permite sobrescrever um valor e com menos checagens,
    // que não são necessárias nesse caso
    public void inserir(Chave chave, Valor valor)
    {
        raiz = inserir(raiz, chave, valor);
    }

    //No método privado não há a opção de sobrescrever o valor de um nó
    private No inserir(No x, Chave chave, Valor valor)
    {
        ...
        else  x.dir = inserir(x.dir, chave, valor);
