...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> 
{
    ...
    */
    ...
    public void transformaBST(Arvbin<Chave> arvoreBinaria){
        ...
        //retorna caso a arvore binaria seja nula
        if (arvoreBinaria == null)
            return;

        //Insere nó raiz da arvore binaria na raiz da arvore binária de busca com o método inserir()
        this.inserir(arvoreBinaria.retornaVal(),null);

        //Acessa a sub-árvore esquerda da árvore binária e passa os valores dela para a árvore binária de busca
        if (arvoreBinaria.retornaEsq() != null)
            acessaSubArvore(arvoreBinaria.retornaEsq());

        //Acessa a sub-árvore direita da árvore binária e passa os valores dela para a árvore binária de busca
        if (arvoreBinaria.retornaDir() != null)
            acessaSubArvore(arvoreBinaria.retornaDir());

    }

    private void acessaSubArvore(Arvbin<Chave> no) {

        //Encerra o método quando a sub-árvore acaba.
        if (no == null)
            return;

        // Percorre pelos nós da esquerda,adicionando o valor do objeto Chave do nó pelo método inserir() e após
        // percorre a subárvore direita.
        if (no.retornaEsq() != null)
            acessaSubArvore(no.retornaEsq());

        if (no.retornaVal() != null)
            inserir(no.retornaVal(), null);

        if (no.retornaDir() != null)
            acessaSubArvore(no.retornaDir());

    }


    //Método put sem a restrição para nós com chave, mas sem valor
    ...
    private No inserir(No x, Chave chave, Valor valor)
    {
        /* Caso base: encontrou a posição de inserção. */
        if (x == null)
            return new No(chave, valor);
